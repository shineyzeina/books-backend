...
function getAll(req, res, next) {
    institutionService.getAll(req.query)
...
function getById(req, res, next) {
    institutionService.getById(req.params.id)
        .then(institution => institution ? res.json(institution) : res.sendStatus(404))
        .catch(err => next(err));
}

function update(req, res, next) {
    
    institutionService.update(req.params.id, req.body)
        .then(() => res.json({}))
        .catch(err => next(err));
}

function _delete(req, res, next) {
    institutionService.delete(req.params.id)
        .then(() => res.json({}))
        .catch(err => next(err));
}